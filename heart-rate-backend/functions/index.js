...
exports.sendAlertEmailV2 = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (metricType === "heartRate") {
      ...
      threshold = userData.spo2Threshold || {min: 95, max: 100};
    } else if (metricType === "stroke") {
      threshold = "";
    // eslint-disable-next-line keyword-spacing
    }else {
      return res.status(400).json({
        success: false,
        message: "metricType không hợp lệ (phải là 'heartRate' hoặc 'spO2' hoặc 'stroke')"});
    }

    console.log("Using threshold:", threshold);

    const metricName = metricType === "heartRate" ? "Nhịp tim" : metricType === "spo2" ? "Nồng độ oxy trong máu" : "Đột quỵ";
    const metricUnit = metricType === "heartRate" ? "bpm" : "%";
    let condition = "";
    if (metricType === "heartRate") {
      condition = value > threshold.max ? "cao" : "thấp";
    } else if (metricType === "spo2") {
      condition = value < threshold.min ? "thấp" : "cao";
    }
    ...
    if (metricType === "heartRate" || metricType === "spo2"){
      ...
      emailContent = `
        <h2>Cảnh báo từ HeartRateApp</h2>
        <p>Kính gửi ${userName},</p>
        <p>Hệ thống đã phát hiện nguy cơ đột quỵ:</p>
        <div 
        style="padding: 15px;
        background-color: #FFF5F5;
        border-left: 4px solid #FF4757;
        margin: 20px 0;">
          <p><strong>
          Nguy cơ đột quỵ của bạn đang ở mức ${value}%
          </strong></p>
        </div>
        <p>Vui lòng kiểm tra lại hoặc liên hệ với 
        chuyên gia y tế nếu tình trạng kéo dài.</p>
    ...
    await admin.firestore().collection("AlertHistory").add({
      ...
      metricType,
      value,
      threshold: metricType === "stroke" ? "" : threshold, // Lưu threshold đã sử dụng
      timestamp: admin.firestore.FieldValue.serverTimestamp(),
      emailSent: true,
