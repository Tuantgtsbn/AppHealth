  ...
  const loadMonthlyData = async (date) => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        if (data.timestamp){
          const dayOfMonth = moment(data.timestamp.toDate()).date().toString();

          if (!dailyData[dayOfMonth]) {
            if (typeof data.heartRate === 'number'){
              dailyData[dayOfMonth].heartRates.push(data.heartRate);
            }
      ...
      if (step > 1 && visibleDays.length > 0 && visibleDays[visibleDays.length - 1] !== daysOfMonth[daysOfMonth.length - 1]) {
      ...
      {loading ? (
        ...
        <ScrollView style={styles.scrollView}>
          <View style={styles.chartContainer}>
            <Text style={styles.chartTitle}>Nhịp tim trung bình (bpm)</Text>
            ...
            <Text style={styles.chartNote}>* Giá trị trung bình hàng ngày</Text>
          </View>

          <View style={styles.chartContainer}>
            <Text style={styles.chartTitle}>SpO2 trung bình (%)</Text>
            {spo2Data.datasets &&
            spo2Data.datasets[0] &&
            ...
            spo2Data.datasets[0].data.length > 0 ? (
              ...
              <View style={styles.noDataContainer}>
                <Text style={styles.noDataText}>Không có dữ liệu SpO2</Text>
              </View>
            )}
